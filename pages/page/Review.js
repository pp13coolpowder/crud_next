...
import { GETPOST, POST, auth, deletepost, GetUserTable } from "../api/axios"
...
export default function Review() {
  ...
  const GetUser = () => {
    ...
    auth(id)
      .then(res => { setDatauser([res.data]), setKom(res.data) })
  ...
  const handlechang = (e) => {
    setpost({ ...post, [e.target.name]: e.target.value, id_user: kom.user_id, user_name: kom.user_name, user_lastname: kom.user_lastname, image: kom.image })
  }
  const submitPost = (e) => {
    e.preventDefault()
    try {
        ...
        {listPost.sort((a, b) => b._id.localeCompare(a._id)).map((item, i) => (
          <div key={i} className="relative mb-5 p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
            <div className="flex justify-between">
              {oll.filter(({ _id }) => _id == item.id_user).map((uu, i) => (
                <div key={i} className="flex py-2 items-center">
                  <diV className="flex py-2 items-center"></diV>
                  <img className="mr-2 w-[30px] h-[30px] rounded-full " src={uu.image} />
                  {uu.first_name}  {uu.last_name}
                  </div>
              ))}
             
              {(idpost != item._id) && (
                <button onClick={() => { if (kom.user_id === item.id_user) { setidpost(item._id) } }}>
                ...
                <div className="absolute right-6 py-3 flex flex-col space-y-1">
                  ...
                  <button onClick={() => {
                    Swal.fire(
                      ...
                      'Delete success',
                      'success'
                    ), deletepost(item._id).then(() => GetData())
                  }} >
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
